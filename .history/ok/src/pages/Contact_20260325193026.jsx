...
const ContactForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let waMessage = `🌟 *NEW JOB APPLICATION* 🌟%0A`;
      waMessage += `────────────────%0A`;
      waMessage += `*👤 Name:* ${escapeWhatsApp(formData.fullName)}%0A`;
      waMessage += `*📧 Email:* ${escapeWhatsApp(formData.email)}%0A`;
      waMessage += `*📞 Phone:* ${escapeWhatsApp(formData.phone)}%0A`;
      waMessage += `*💼 Job Field:* ${escapeWhatsApp(formData.jobField)}%0A`;
      
      if (formData.message) {
        waMessage += `*📝 Note:* ${escapeWhatsApp(formData.message)}%0A`;
      }
      
      waMessage += `*📄 Resume:* ${escapeWhatsApp(resumeFile.name)} (${(resumeFile.size / 1024).toFixed(1)} KB)%0A`;
      
      if (resumeLink) {
        waMessage += `*🔗 Download Resume:* ${resumeLink}%0A`;
        waMessage += `_⚠️ Link valid while this session is active. Please download immediately._%0A`;
      }
      
      waMessage += `────────────────%0A`;
      waMessage += `📅 Submitted via Career Connect Portal%0A`;
      waMessage += `🔔 Please contact candidate at ${escapeWhatsApp(formData.phone)}`;
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          if (resumeLink) URL.revokeObjectURL(resumeLink);
        }, 120000);
      }, 800);
